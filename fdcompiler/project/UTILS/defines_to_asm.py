...
def runMain(output, input):
	getStringForFile = lambda name : "\n".join(["; " + ("="*23) + "+" + ("-"*len(name)) + "+", f"; Start of contents from |{name}|", "; " + ("="*23) + "+" + ("-"*len(name)) + "+", "", ""])
	rootPath = pathlib.Path('/project/UTILS/')
	outfile = open(rootPath / pathlib.Path(output).with_suffix(".s"), "w")
	for filename in input:
		outfile.write(getStringForFile(filename))
		realFilename = rootPath / filename
		convertCFileToS(realFilename, outfile)

runMain('/project/TMP/main/BUILD_FLAGS.s', ['/project/BUILD_FLAGS.h','/project/LEVELS/include/lvlset_A/level_defines.h','/project/SAUCE/defines/space_defines.h','/project/SAUCE/defines/physics_defines.h'])