...
def main(csvFolder, metadata, outputFolder):

	if not metadata.is_file():
		print("Path to the metadata json5 file is invalid. Please specify a proper file.")
		exit(1)

	with metadata.open() as metafile:
		metadata = json5.load(metafile)
	...
	include_path = (outputFolder).resolve()
	if not include_path.is_dir():
		include_path.mkdir(parents=True)

	if len(levels) > 0:
		bg_exp_data = export_bg(csvFolder, filteredMetadata, include_path)
		spr_exp_data = export_spr(csvFolder, filteredMetadata, globalObjectOffsetSettings)
	else:
		bg_exp_data = [[], [], []]
		spr_exp_data = [[], None]
